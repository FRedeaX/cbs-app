...
export const ImageViewerZoom = forwardRef<
  ImageViewerZoomImperativeRef,
  ImageViewerZoomProps
  ...
  useGesture(
    {
      ...
      onDrag: (state) => {
        ...
        // Жест начался, блокируем ось, если допустимое смещение равно 0
        if (first) {
          const root = rootRef.current;
          const target = animatedRef.current;
          if (root === null || target === null) return memo;

          const [offsetX, offsetY] = getOffset(style.scale.get(), target, root);
